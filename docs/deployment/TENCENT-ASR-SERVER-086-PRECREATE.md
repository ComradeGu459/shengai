# TENCENT-ASR-SERVER-086-PRECREATE

## outcome

blocked。唯一 `PreCreateDiagnostic` Execute 未达到 `precreate_ready`；`business.run` 返回精确稳定码 `BUDGET_CREATE_API_SYSTEM_CONTROL_FORBIDDEN`。未执行 full Execute。

## evidence

- RunId：`asr-086-20260829-r1`
- diagnostic plan SHA：`adb2a586452676af1c2d7f65c0dd32d72f0d96f3393cea300a5252449c4f8979`
- Preflight：通过；模式为 `PreCreateDiagnostic`，Tencent writes、CreateRecTask 上限均为 0，停止点为 `AsrWorker.runOnce()` 前。
- Execute：`outcome=blocked`、`code=BUDGET_CREATE_API_SYSTEM_CONTROL_FORBIDDEN`、`stage_code=BUDGET_CREATE_API_SYSTEM_CONTROL_FORBIDDEN`。
...
唯一业务缺口是 `BUDGET_CREATE_API_SYSTEM_CONTROL_FORBIDDEN`，定位在预算创建 API 的 system-control 权限门；未进入 AsrWorker，Tencent 写入为 0。按停止条件不重试、不换 run、不执行 full。
