...
[ "$inbound" = '/var/tmp/qimao-ocr-media-error-1684-inbound' ] \
  || input_fail 'INBOUND_PATH_INVALID'

archive_name='ocr-media-error-20260831-r1.tar.gz'
...
lock_file='/run/lock/qimao-ocr-media-error-1684.lock'
exec 9>"$lock_file"
flock -n 9 || input_fail 'LOCK_ALREADY_HELD'

old_release='/opt/qimao-terms-cloud/releases/ocr-worker-once-20260831-r1'
new_release='/opt/qimao-terms-cloud/releases/ocr-media-error-20260831-r1'
release_stage='/opt/qimao-terms-cloud/releases/.ocr-media-error-20260831-r1-stage-1684'
current='/opt/qimao-terms-cloud/current'
current_next='/opt/qimao-terms-cloud/current.ocr-media-error-1684.next'
static_link='/srv/qimao-terms-cloud/frontend'
old_static='/srv/qimao-terms-cloud/frontend-asr-srt-compare-20260831-r2'
runtime='/run/qimao-ocr-media-error-1684'
...
(
  ...
  runuser -u qimao -- /usr/local/bin/node --input-type=module -e "
    ...
    const { ScreenTextMediaError } = await import('./dist/modules/screen-text/screen-text-media.js');
    ...
    const typedError = new ScreenTextMediaError('UNKNOWN');
    if (typedError.code !== 'UNKNOWN' || typedError.message !== 'SCREEN_TEXT_MEDIA_UNKNOWN') process.exit(1);
  "
)
echo 'candidate_import_gate=passed qimao:imports+timeouts(60000/61000/62000)+worker_mode(continuous/once/reject-extra)+media_error(UNKNOWN)'
...
safe_remove "$runtime"
trap - ERR EXIT INT TERM
echo "terminal=passed current:$new_release backend_restart:1 openvino_restart:0 timeouts:60000/61000/62000 worker_mode:once+continuous media_error:typed-unknown asr_restart:0 screen_start:0 db_write:0 cos_write:0 route_write:0 budget_write:0 provider_calls:0"
exit 0
